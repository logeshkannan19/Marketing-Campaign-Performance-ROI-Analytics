...
Interactive Streamlit dashboard for exploring marketing campaign performance.
Provides KPI overview, channel analysis, and campaign insights.
"""
...
def plot_roi_by_channel(df: pd.DataFrame) -> px.bar:
    """Create ROI by channel visualization."""
    ...
    channel_summary["roi"] = (
        channel_summary["revenue"] - channel_summary["cost"]
    ) / channel_summary["cost"]
...
def plot_top_campaigns(df: pd.DataFrame, n: int = 10) -> px.bar:
    """Create top campaigns by revenue visualization."""
    ...
    campaign_summary["roi"] = (
        campaign_summary["revenue"] - campaign_summary["cost"]
    ...
    fig = px.bar(
        ...
        x="revenue",
        orientation="h",
        title=f"Top {n} Campaigns by Revenue",
        color="roi",
...
def plot_monthly_trends(df: pd.DataFrame) -> px.line:
    """Create monthly spend vs revenue trend visualization."""
    df_copy = df.copy()
...
def plot_channel_scatter(df: pd.DataFrame) -> px.scatter:
    """Create channel performance scatter plot."""
    channel_summary = df.groupby("channel").agg({
        "ctr": "mean",
    ...
    fig = px.scatter(
        ...
        size="cost",
        color="channel",
        title="Channel Performance: CTR vs Conversion Rate",
        labels={
...
def main() -> None:
    ...
    st.markdown(
        "Track ROI, analyze channel performance, and identify optimization opportunities."
    )

    st.sidebar.markdown("---")
    st.sidebar.caption(
        f"Displaying {len(filtered_df):,} of {len(df):,} records"
    )
    ...
    display_cols = [
        "campaign_name", "channel", "region", "date",
    ...
    st.markdown("---")
    st.caption(
        f"Data period: {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}"
    )
